...
describe('parseDate()', () => {
	it('parses ISO datetime (YYYY-MM-DDTHH:MM)', () => {
		...
		expect(
			(result as { type: 'datetime'; date: Date }).date.getTime(),
		).not.toBeNaN();
	});
...
describe('deadlineToCalendarDay()', () => {
	it('converts date-only deadline', () => {
		...
		expect(deadlineToCalendarDay(d)).toEqual({
			year: 2026,
			month: 12,
			day: 25,
		});
...
describe('addDaysToCalendarDay()', () => {
	it('adds positive days', () => {
		expect(
			addDaysToCalendarDay({ year: 2026, month: 6, day: 11 }, 5),
		).toEqual({ year: 2026, month: 6, day: 16 });
	});

	it('adds negative days', () => {
		expect(
			addDaysToCalendarDay({ year: 2026, month: 6, day: 11 }, -3),
		).toEqual({ year: 2026, month: 6, day: 8 });
	});

	it('wraps into next month', () => {
		expect(
			addDaysToCalendarDay({ year: 2026, month: 6, day: 28 }, 5),
		).toEqual({ year: 2026, month: 7, day: 3 });
	});

	it('wraps into previous month', () => {
		expect(
			addDaysToCalendarDay({ year: 2026, month: 6, day: 1 }, -3),
		).toEqual({ year: 2026, month: 5, day: 29 });
	});

	it('wraps into next year', () => {
		expect(
			addDaysToCalendarDay({ year: 2026, month: 12, day: 30 }, 3),
		).toEqual({ year: 2027, month: 1, day: 2 });
	});

	it('handles zero days', () => {
		expect(
			addDaysToCalendarDay({ year: 2026, month: 6, day: 11 }, 0),
...
describe('taskDeadlineOnCalendarDay()', () => {
	...
	it('returns true when datetime matches reference date', () => {
		const d: Deadline = {
			type: 'datetime',
			date: new Date('2026-06-11T15:00:00'),
		};
		expect(taskDeadlineOnCalendarDay(d, new Date('2026-06-11'))).toBe(true);
	});
	...
	it('returns false when dates differ', () => {
		...
		expect(taskDeadlineOnCalendarDay(d, new Date('2026-06-11'))).toBe(
			false,
		);
	});
...
describe('taskDeadlineOverdueBeforeDay()', () => {
	it('returns true when deadline is before reference date', () => {
		...
		expect(taskDeadlineOverdueBeforeDay(d, new Date('2026-06-11'))).toBe(
			true,
		);
	});
...
describe('parseTaskLine()', () => {
	...
	it('parses due:: prefix date', () => {
		const result = parseTaskLine(
			'- [ ] Quarterly review due:: 2026-07-01',
			'test.md',
			7,
		);
		expect(result).not.toBeNull();
	...
	it('parses scheduled:: prefix date', () => {
		const result = parseTaskLine(
			'- [ ] Plan vacation scheduled:: 2026-08-15',
			'test.md',
			9,
		);
		expect(result).not.toBeNull();
	...
	it('parses starts:: prefix date', () => {
		const result = parseTaskLine(
			'- [ ] Start project starts:: 2026-09-01',
			'test.md',
			11,
		);
		expect(result).not.toBeNull();
...
describe('Inline task time parsing (issue #88)', () => {
	...
	it('parses seconds-precision time but reports HH:MM in timeString', () => {
		const result = parseTaskLine(
			'- [ ] Call Grandma 📅 2026-07-22 12:30:45',
			'note.md',
			1,
		);
		expect(result).not.toBeNull();
	...
	it('parses due:: with time as datetime', () => {
		const result = parseTaskLine(
			'- [ ] Quarterly review due:: 2026-07-22 09:00',
			'note.md',
			1,
		);
		expect(result).not.toBeNull();
	...
	it('parses scheduled:: with time as datetime', () => {
		const result = parseTaskLine(
			'- [ ] Plan trip scheduled:: 2026-07-22 14:00',
			'note.md',
			1,
		);
		expect(result).not.toBeNull();
	...
	it('parses starts:: with time as datetime', () => {
		const result = parseTaskLine(
			'- [ ] Start project starts:: 2026-07-22 08:15',
			'note.md',
			1,
		);
		expect(result).not.toBeNull();
...
describe('Reminder-plugin inline syntax (issue #96)', () => {
	...
	it('parses the time component of recurring reminder syntax and ignores the suffix', () => {
		const result = parseTaskLine(
			'- [ ] Call Grandma #Task/regularly (@2026-07-31 09:00 🔁 every week on Sunday)',
			'note.md',
			1,
		);
		expect(result).not.toBeNull();
	...
	it('treats bare @YYYY-MM-DD as date-only only via Kanban syntax (issue #97)', () => {
		...
		const enabled = parseTaskLine(
			'- [ ] Buy milk @2026-07-22',
			'note.md',
			1,
		);
		expect(enabled).not.toBeNull();
		expect(enabled!.deadline).toEqual({
			type: 'date-only',
			year: 2026,
			month: 7,
			day: 22,
		...
		const disabled = parseTaskLine(
			'- [ ] Buy milk @2026-07-22',
			'note.md',
			1,
			{ kanbanSyntaxEnabled: false },
		);
	...
	it('parses all variants via parseInlineTasks (end-to-end)', () => {
		...
		const parenDateOnly = tasks.find((t) =>
			t.text.includes('Paren date-only'),
		)!;
		...
		expect(parenDateOnly.deadline).toEqual({
			type: 'date-only',
			year: 2026,
			month: 8,
			day: 2,
		});
		expect(plain.deadline).toEqual({
			type: 'date-only',
			year: 2026,
			month: 8,
			day: 3,
	...
	it('skips reminder syntax only when parseInlineTasks is told to', () => {
		...
		const enabled = parseInlineTasks(content, 'note.md', 0, {
			reminderSyntaxEnabled: true,
		});
		const disabled = parseInlineTasks(content, 'note.md', 0, {
			reminderSyntaxEnabled: false,
			kanbanSyntaxEnabled: false,
		});
...
describe('Kanban-plugin inline syntax (issue #97)', () => {
	...
	it('does not treat @date with a non-time @@ suffix as a deadline', () => {
		const result = parseTaskLine(
			'- [ ] Call Grandma @2026-07-22 @@tomorrow',
			'note.md',
			1,
		);
		expect(result).not.toBeNull();
	...
	it('parses all Kanban variants via parseInlineTasks (end-to-end)', () => {
		...
		const tasks = parseInlineTasks(content, 'note.md');
		expect(tasks).toHaveLength(4);
		const kanbanAtTime = tasks.find((t) =>
			t.text.includes('Kanban at-time'),
		)!;
		const kanbanDateOnly = tasks.find((t) =>
			t.text.includes('Kanban date-only'),
		)!;
		const reminder = tasks.find((t) =>
			t.text.includes('Reminder at-time'),
		)!;
		...
		expect(kanbanDateOnly.deadline).toEqual({
			type: 'date-only',
			year: 2026,
			month: 8,
			day: 2,
		...
		expect(obsidian.deadline).toEqual({
			type: 'date-only',
			year: 2026,
			month: 8,
			day: 4,
...
describe('parseFrontmatterTasksFromCache()', () => {
	...
	it('parses a frontmatter task with scheduled date', () => {
		...
		expect(result[0]!.deadline).toEqual({
			type: 'date-only',
			year: 2026,
			month: 6,
			day: 11,
		...
		const result = parseFrontmatterTasksFromCache(
			{
				scheduled: '2026-06-11',
				status: 'open',
				tags: 'work, urgent' as unknown as string[],
			},
			content,
			'test.md',
		);
...
describe('scanVaultForTasks()', () => {
		...
		app.metadataCache.getFileCache.mockImplementation(
			(file: { path: string }) => {
				...
				return {
					...
					frontmatterPosition: { end: { line: 3 } },
				};
			},
		);
	...
	it('respects reminderSyntaxEnabled when scanning (issue #96)', async () => {
		...
		const app = createMockApp([
			{ path: 'inbox.md', content, frontmatter: undefined },
		]);

		const enabled = await scanVaultForTasks(app, {
		...
		const disabled = await scanVaultForTasks(app, {
			scanMode: 'whole-vault',
			targetFolder: '',
			reminderSyntaxEnabled: false,
			kanbanSyntaxEnabled: false,
	...
	it('respects kanbanSyntaxEnabled when scanning (issue #97)', async () => {
		...
		const kanbanAtTime = enabled.find((t) =>
			t.text.includes('Kanban at-time'),
		)!;
		expect(kanbanAtTime.deadline!.type).toBe('datetime');
		expect(kanbanAtTime.timeString).toBe('09:30');
		const kanbanDateOnly = enabled.find((t) =>
			t.text.includes('Kanban date-only'),
		)!;
...
describe('filterDueTasksByCheckFlags()', () => {
	...
	it('returns all tasks when both flags are true', () => {
		const result = filterDueTasksByCheckFlags(
			[todayTask, overdueTask],
			refDate,
			true,
			true,
		);
...
describe('getUpcomingTasks()', () => {
	...
	it('returns tasks within the next N days', () => {
		const tasks = [
			makeInlineTask({
				id: 't1',
				deadline: makeDeadlineDateOnly(2026, 6, 12),
			}), // tomorrow
			makeInlineTask({
				id: 't2',
				deadline: makeDeadlineDateOnly(2026, 6, 18),
			}), // 7 days ahead
			makeInlineTask({
				id: 't3',
				deadline: makeDeadlineDateOnly(2026, 6, 19),
			}), // 8 days ahead — excluded
		];
...
describe('getTaskNotificationKey()', () => {
	it('returns a key with notified prefix, task id, and date', () => {
		const task = makeInlineTask({
			id: 'inline:file.md:2026-06-11:abc',
			deadline: makeDeadlineDateOnly(2026, 6, 11),
		});
		expect(getTaskNotificationKey(task)).toBe(
			'notified:inline:file.md:2026-06-11:abc:2026-06-11',
		);
	});

	it('uses "unknown" for deadline-less tasks', () => {
		const task = makeInlineTask({
			id: 'inline:file.md:unknown:abc',
			deadline: null,
		});
		expect(getTaskNotificationKey(task)).toBe(
			'notified:inline:file.md:unknown:abc:unknown',
		);
	});
});
			...
			if (taskDue) {
				...
				const expectedDue =
					compareCalendarDays(
						...
						{
							year:
								task.deadline!.type === 'date-only'
									? task.deadline!.year
									: task.deadline!.date.getFullYear(),
							month:
								task.deadline!.type === 'date-only'
									? task.deadline!.month
									: task.deadline!.date.getMonth() + 1,
							day:
								task.deadline!.type === 'date-only'
									? task.deadline!.day
									: task.deadline!.date.getDate(),
						},
					) >= 0;
...
describe('parseRecurrence()', () => {
	it('parses "🔁 every day"', () => {
		expect(parseRecurrence('🔁 every day')).toEqual({
			period: 'day',
			raw: 'every day',
		});
	});
	...
	it('parses "🔁 every week on Sunday" with weekday', () => {
		expect(parseRecurrence('🔁 every week on Sunday')).toEqual({
			period: 'week',
			weekday: 0,
			raw: 'every week on Sunday',
		});
	});

	it('parses weekday abbreviations (case-insensitive)', () => {
		expect(parseRecurrence('🔁 every WEEK on sat')).toEqual({
			period: 'week',
			weekday: 6,
			raw: 'every WEEK on sat',
		});
	});

	it('parses "🔁 every month"', () => {
		expect(parseRecurrence('🔁 every month')).toEqual({
			period: 'month',
			raw: 'every month',
		});
	});
	...
	it('ignores "on <weekday>" for non-week periods', () => {
		expect(parseRecurrence('🔁 every month on Sunday')).toEqual({
			period: 'month',
			raw: 'every month on Sunday',
		});
	});

	it('falls back to plain weekly when the weekday name is unknown', () => {
		expect(parseRecurrence('🔁 every week on Foop')).toEqual({
			period: 'week',
			raw: 'every week on Foop',
		});
	});
...
describe('computeNextOccurrence()', () => {
	...
	it('week on Sunday: next Sunday strictly after the deadline', () => {
		...
		const next = computeNextOccurrence(
			d,
			{ period: 'week', weekday: 0, raw: 'every week on Sunday' },
			new Date('2026-07-22T12:00:00'),
		)!;
	...
	it('month: same day next month, clamped to month length', () => {
		...
		expect(
			deadlineToDateString(
				computeNextOccurrence(
					parseDate('2028-01-31')!,
					{ period: 'month', raw: 'every month' },
					new Date('2028-01-31T12:00:00'),
				),
			),
		).toBe('2028-02-29');
	});
...
describe('buildNextOccurrenceLine() (issue #98)', () => {
	it('rewrites the full reminder-plugin syntax, unchecking the box and preserving text', () => {
		const task = parseTaskLine(
			'- [x] Call Grandma (@2026-07-31 09:00 🔁 every week on Sunday)',
			'note.md',
			1,
		)!;
		const built = buildNextOccurrenceLine(
			task,
			new Date('2026-07-31T12:00:00'),
		);
		expect(built).not.toBeNull();
		expect(built!.line).toBe(
			'- [ ] Call Grandma (@2026-08-02 09:00 🔁 every week on Sunday)',
		);
	});
	...
	it('returns null for tasks without recurrence', () => {
		const task = parseTaskLine(
			'- [x] Water plants 📅 2026-07-22',
			'note.md',
			1,
		)!;
		expect(
			buildNextOccurrenceLine(task, new Date('2026-07-22T12:00:00')),
		).toBeNull();
	});
});
...
describe('computeCompletionReschedules() (issue #98)', () => {
	...
	it('emits an edit when a recurring task transitions to completed', () => {
		...
		expect(edits[0]!.oldLine).toBe(
			'- [x] Water plants 📅 2026-07-22 🔁 every day',
		);
		expect(edits[0]!.newLine).toBe(
			'- [ ] Water plants 📅 2026-07-23 🔁 every day',
		);
	});
	...
	it('emits no edits without recurrence', () => {
		const before = [
			parseTaskLine('- [ ] Water plants 📅 2026-07-22', 'note.md', 1)!,
		];
		const after = [
			parseTaskLine('- [x] Water plants 📅 2026-07-22', 'note.md', 1)!,
		];
