...
export const Setting = MockSetting as unknown as ReturnType<typeof vi.fn> &
	(new (container: HTMLElement) => MockSetting);
 ...
 */
function createObsidianElement(
	tag: string,
...
function attachObsidianExtensions(el: HTMLElement): void {
	if ((el as unknown as { __obsidianExtended?: boolean }).__obsidianExtended)
		return;
	(el as unknown as { __obsidianExtended: boolean }).__obsidianExtended =
		true;
	...
	(el as unknown as { removeClass: (c: string) => void }).removeClass =
		vi.fn();
	(
		el as unknown as { setAttr: (attr: string, value: string) => void }
	).setAttr = vi.fn();
	(el as unknown as { setText: (text: string) => void }).setText = vi.fn(
		(text: string) => {
			el.textContent = text;
		},
	);
	(
		...
		(tag: string, opts?: { cls?: string; text?: string }) => {
	...
	).createDiv = vi.fn((opts?: { cls?: string; text?: string }) => {
